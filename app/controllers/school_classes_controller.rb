class SchoolClassesController < ApplicationController
  ...
  def new
    ...
    if @school.school_id != nil
      @school.idcreatepl = @school.school_id
    elsif @school.school_id == nil
      @school.idcreatepl = @school.id
    end
    #@teacherstoselect = Teacher.joins("INNER JOIN school_teachers ON school_teachers.teacher_id = teachers.id INNER JOIN schools ON schools.id = school_teachers.school_id where schools.id = #{@schoolid} or schools.school_id = #{@schoolid} ")
    #nombre de room
